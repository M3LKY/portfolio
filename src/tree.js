...
function initThree() {
  ...
  // Create OrbitControls after the camera is defined
  orbit = new OrbitControls(camera, renderer.domElement);
  orbit.enableDamping = true;
  orbit.dampingFactor = 0.05;
  orbit.screenSpacePanning = false;
  orbit.minDistance = 1;
  orbit.maxDistance = 5384;
...
async function render(galaxy) {
  ...
  camera.aspect = canvas.clientWidth / canvas.clientHeight;
  camera.updateProjectionMatrix();
  galaxy.group.rotation.x += 0.005; // Adjust the rotation speed as desired
  galaxy.group.rotation.y += 0.005; // Adjust the rotation speed as desired
  
  //zoom it move it to see his beauty
  ...
  requestAnimationFrame(() => render(galaxy)); // Pass the galaxy object as a parameter
}
